...
#         self.assertLess(np.abs((dSapprox-dSexact)/dSexact), 0.001)
    ...
    def test_linVnln_dZeta(self):
        ...
        print(f_dZeta)
        print(dfApprox)
        print(dfApprox-f_dZeta)
        
        # lift
        dLexact = sum(f_dZeta[2:len(f_dZeta)/2:3])
        dLapprox = sum(dfApprox[2:len(f_dZeta)/2:3])
        print(dLexact)
        print(dLapprox)
#         self.assertLess(np.abs((dLapprox-dLexact)/dLexact), 0.001)
        
        # drag
        dDexact = sum(f_dZeta[0:len(f_dZeta)/2:3])
        dDapprox = sum(dfApprox[0:len(f_dZeta)/2:3])
        print(dDexact)
        print(dDapprox)
#         self.assertLess(np.abs((dDapprox-dDexact)/dDexact), 0.001)
        
        # side force
        dSexact = sum(f_dZeta[1:len(f_dZeta)/2:3])
        dSapprox = sum(dfApprox[1:len(f_dZeta)/2:3])
        print(dSexact)
        print(dSapprox)
#         self.assertLess(np.abs((dSapprox-dSexact)/dSexact), 0.001)
