...
shape=load('DANC_SOL912_shape.txt');
rigid=load('DANC_SOL912_rigid.txt');
...
for i=1:1:length(Time)-1,
    ...
    left(1:lr-1,:)= shape((i-1)*NumN+lr+1:i*NumN,2:4);
    left(lr,:)    = shape((i-1)*NumN+1,2:4);
    
    for j=1:size(right,1),
        right(j,:)=(R*right(j,:)'+S(i,1:3)')';
        left(j,:) =(R*left(j,:)' +S(i,1:3)')'; %#ok<*AGROW>
    end
    
    tips(i,:)=right(end,:);

    figure(4)
    plot3(right(:,1),right(:,2),right(:,3),'-b','LineWidth',2)
%    plot3(right(:,1),right(:,2),right(:,3),'-xb')
%    plot(right(:,1),right(:,3),'-b','LineWidth',2)
    set(gcf,'Color',[1,1,1])
    hold on
    plot3(left(:,1),left(:,2),left(:,3),'-b','LineWidth',2)
%    plot3(left(:,1),left(:,2),left(:,3),'-xr')
%    plot(left(:,1),left(:,3),'-r','LineWidth',2)
    axis equal
    axis([-18 18 -18 100 -18 18])
    grid
    hold off
    pause(0.01)
%    F(i/1) = getframe(gcf);
...
%movie2avi(F,'motion','compression','None')
