!-> Program.- MAIN - 06Jan2011 Updated: 07/01/2011
!
...
&                            PosDef,PsiDef,Options)
  ...
  ! Dynamic solution.
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  if ((Options%Solution.ge.200).and.(Options%Solution.le.922)) then
    ...
    allocate (PosPsiTime(NumSteps,6));         PosPsiTime=0.d0
    allocate (VelocTime (NumSteps,NumNodes));  VelocTime= 0.d0
    ...
    case (202,322)
      allocate (DynOut (NumSteps*NumNodes,3)); DynOut   =0.d0
...
&                              OutGrids,Options)
    ...
    ! CBEAM3: Nonlinear dynamic (312: around the current deformed beam).
    case (212,312)
      allocate (DynOut (NumSteps*NumNodes,3)); DynOut    = 0.d0
      call cbeam3_solv_nlndyn (12,NumDof,Time,Elem,Node,ForceStatic*0.d0,ForceDynAmp,         &
&                              ForceTime,ForcedVel,ForcedVelDot,PosIni,PsiIni,                &
&                              PosDef,PsiDef,PosDotDef,PsiDotDef,PosPsiTime,VelocTime,DynOut, &
&                              OutGrids,Options)
    end select
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    if ((Options%Solution.ge.900).and.(Options%Solution.le.922)) then
    
      ! Initialize
      allocate (RefVel   (NumSteps+1,6));      RefVel   =ForcedVel
      allocate (RefVelDot(NumSteps+1,6));      RefVelDot=ForcedVelDot
      allocate (Quat     (4));                 Quat     =(/1.d0,0.d0,0.d0,0.d0/)    !(/0.707106781186547d0, 0.d0,0.707106781186547d0, 0.d0/)
      allocate (DynOut (NumSteps*NumNodes,3)); DynOut   =0.d0
...
&                                   ForceTime,RefVel,RefVelDot,PosIni,PsiIni,                &
&                                   PosDef,PsiDef,PosDotDef,PsiDotDef,PosPsiTime,VelocTime,  &
&                                   OutGrids,Options)
...
&                                      OutGrids,Options)
      ...
      ! Store rigid body velocities of global reference frame
...
! Store results for general dynamic analysis.
    ...
    open (unit=11,file=OutFile(1:11)//'_dyn.txt',status='replace')
      do i=1,NumSteps;  write (11,'(1X,1P7E15.6)') Time(i),PosPsiTime(i,:);  end do
    close (11)
