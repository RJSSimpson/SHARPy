...
! Local variables.
  ...
  real(8) :: ShapeFun(MaxNodCB3)    ! Shape functions in a z.
  real(8) :: ShapeDer(MaxNodCB3)    ! Derivatives of ShapeFun in a z.

  real(8) :: Ri_z (6)              ! Displacement/rotation at z.
  real(8) :: r0_z (6)              ! Position vector at z.
  real(8) :: dRi_z(6)              ! Derivative of Ri_z.
  real(8) :: dr0_z(6)              ! Derivative of r0_z.

  real(8) :: CBa(3,3)              ! Coordinate transformation matrix.
  real(8) :: KB(3)                 ! Curvature in the deformed configuration at z.
  real(8) :: gamma(3)              ! Strain at z.
  real(8) :: Rot(3,3),dRot(3,3)    ! Tangential operator and spatial derivative.
  real(8) :: Strain(6)             ! Beam strains at the z.
...
! Global coordinates and their derivatives at z.
    do i=1,6
...
! Compute force/moment strains.
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!-> Subroutine CBEAM3_STRAINZ
!
!-> Description:
!
!    Compute strain local element ordinate z.
!
!-> Remarks.-
!
!   1) For three noded elements r0(i,:) corresponds to LHS elemnt for i=1, RHS
...
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 subroutine cbeam3_strainz (NumNodesElem,r0,Ri,Strain,z)
  use lib_fem
  use lib_rot
  use lib_rotvect
...
! I/O Variables.
  ...
  real(8),intent(in)   :: r0       (NumNodesElem,6)! Initial position/orient
  real(8),intent(in)   :: Ri       (NumNodesElem,6)! Current position/orient
  real(8),intent(out):: Strain   (6)       ! Stiffness forces.
  real(8),intent(in)   :: z                  ! Local element coordinate
...
! Local variables.
  ...
  real(8) :: Ri_z (6)              ! Displacement/rotation at z.
  real(8) :: r0_z (6)              ! Position vector at the z.
  real(8) :: dRi_z(6)              ! Derivative of Ri_z.
  real(8) :: dr0_z(6)              ! Derivative of r0_z.
...
! Global coordinates and their derivatives at the zs.
    do i=1,6
      r0_z(i) = dot_product (ShapeFun(1:NumNodesElem),r0(1:NumNodesElem,i))
      dr0_z(i)= dot_product (ShapeDer(1:NumNodesElem),r0(1:NumNodesElem,i))
    end do
